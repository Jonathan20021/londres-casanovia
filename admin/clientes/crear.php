<?php
/**
 * Crear cliente — LONDRES Casa de Novias
 */
...
if (is_post()) {
    ...
    if (!$errors) {
        ...
        log_activity('create', 'customer', $id, 'Cliente creado: ' . $form['full_name']);
...
$page_subtitle = 'Registra los datos de contacto del cliente.';
$active        = 'clientes';
$header_actions = '<a href="' . admin_url('clientes/index.php') . '" class="inline-flex items-center gap-2 rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-sm font-medium text-gray-700 transition hover:bg-gray-50">'
...
<form method="post" class="mx-auto max-w-3xl space-y-6">
        ...
        <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
            <div class="sm:col-span-2">
                ...
                <input type="text" id="full_name" name="full_name" value="<?= e($form['full_name']) ?>" required maxlength="150"
                       class="lcn-input <?= isset($errors['full_name']) ? 'border-rose-400 focus:border-rose-400 focus:ring-rose-400/20' : '' ?>" autofocus>
            ...
            <div>
                ...
                <input type="date" id="birthdate" name="birthdate" value="<?= e($form['birthdate']) ?>"
    ...
    <div class="flex flex-col-reverse items-stretch gap-3 sm:flex-row sm:items-center sm:justify-end">
        <a href="<?= admin_url('clientes/index.php') ?>" class="inline-flex items-center justify-center gap-2 rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-sm font-medium text-gray-700 transition hover:bg-gray-50">Cancelar</a>
        <button type="submit" class="inline-flex items-center justify-center gap-2 rounded-xl bg-brand-red px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-red-700">
            <?= icon('check', 'w-4 h-4') ?> Guardar cliente
        </button>
    </div>
</form>
