...
CREATE TABLE `rental_items` (
    ...
    `needs_alteration`   TINYINT(1) NOT NULL DEFAULT 0,
    `alteration_notes`   TEXT NULL,
    `alteration_status`  ENUM('pending','done') NOT NULL DEFAULT 'pending',
    `alteration_done_at` DATETIME NULL,
    `alteration_done_by` INT UNSIGNED NULL,
    `sort_order` INT NOT NULL DEFAULT 0,
    `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`),
    UNIQUE KEY `uq_rental_items_product` (`rental_id`, `product_id`),
    KEY `idx_rental_items_product` (`product_id`, `rental_id`),
    KEY `idx_ri_alteration` (`needs_alteration`, `alteration_status`),
    ...
    CONSTRAINT `fk_ri_alteration_user` FOREIGN KEY (`alteration_done_by`) REFERENCES `users`(`id`) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE `business_settings` (
    ...
    `late_fee_per_day`           DECIMAL(12,2) NOT NULL DEFAULT 500.00,
    `late_fee_workweek`          ENUM('mon_fri','mon_sat') NOT NULL DEFAULT 'mon_fri',
