...
function product_card(array $p): string
{
    ...
    $rental = isset($p['rental_price']) ? money($p['rental_price']) : null;
    $sale   = !empty($p['sale_price']) ? money($p['sale_price']) : null;
    ...
    $html .= '<div>';
    if ($rental) $html .= '<p class="text-xs text-gray-400">Alquiler desde</p><p class="text-lg font-semibold text-gray-900">' . e($rental) . '</p>';
    if ($sale)   $html .= '<p class="mt-0.5 text-xs text-gray-400">Venta ' . e($sale) . '</p>';
    $html .= '</div></div>';
